...
.tbtn{position:absolute;pointer-events:auto;width:68px;height:68px;border-radius:18px;border:2px solid #ffffff55;background:#111b;color:white;font-size:30px;font-weight:bold;user-select:none;-webkit-user-select:none;touch-action:none}
#left{left:20px;bottom:24px} #right{left:100px;bottom:24px}
#jump{right:100px;bottom:24px} #interact{right:20px;bottom:24px}
#pauseTouch{right:18px;top:18px;width:52px;height:52px;font-size:20px}
...
const player={
 ...
 gravity:.7,jump:-14,speed:4,onGround:true,
...
function resetPlayer(){
 player.x=100;player.y=350;player.vy=0;player.onGround=true;
 camera=0;
}
...
function updateGame(dt){
 let moving=false;
 if(keys.left){player.x-=player.speed;moving=true}
 if(keys.right){player.x+=player.speed;moving=true}
 player.x=clamp(player.x,0,WORLD-player.w);
 ...
 // Caixa: plataforma e bloqueio lateral.
 ...
 if(keys.interact && intersects(pr,door)){
   keys.interact=false;
   state="scenario2";
   resetPlayer();
 }
...
function drawBackground(){
 ...
 rect(-camera,420,WORLD,80,"#373943");
...
function drawPlayer(){
 ...
 if(!drawImg(name,x,y,60,70,keys.left)){
...
function drawWorld(){
 ...
 if(!drawImg("porta2.png",door.x-camera,door.y,100,150)){
   rect(door.x-camera,door.y,100,150,"#8a4f18");
 ...
 text("Explora o laboratório!",20,32,20,"#fff");
 text("A/D mover  •  ESPAÇO saltar  •  E entrar",450,32,18,"#00ffc8","center");
}

function drawScenario2(){
 rect(0,0,W,H,"#141824");
 drawImg("fundo2_tutorial.webp",-camera,0,1000,500);
 rect(0,420,W,80,"#373943");
 drawPlayer();
 text("CENÁRIO 2",20,32,22,"#00ffc8");
 text("ESC pausa",20,58,16,"#fff");
}
...
function drawMenu(){
 ...
 text("DESTROY ALIENS",W/2,160,48,"#00ffc8","center");
 text("Uma aventura no laboratório",W/2,195,20,"#fff","center");
 button(350,235,200,60,"JOGAR","#009b60");
 text("PC: teclado  |  Telemóvel: botões no ecrã",W/2,350,16,"#bbb","center");
}
...
function updateCutscene(){
 cutTimer++;
 // A cutscene avança por teclas; este timer só impede ficar totalmente parado.
}
...
function updateConstruction(){
 cutTimer++;
 if(cutTimer>250){state="game";resetPlayer()}
}

function handlePointer(x,y){
 if(state==="menu"){
   if(x>=350&&x<=550&&y>=235&&y<=295) startGame();
 }else if(state==="pause"){
...
function bindButton(id,key,once=false){
 ...
 const down=e=>{e.preventDefault();if(once){keys[key]=true}else keys[key]=true};
...
function loop(now){
 ...
 if(state==="scenario2")updateGame(dt);
